...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
		cardDrawn = 0;
      return playAdventurer(state, drawntreasure, currentPlayer, cardDrawn, z, temphand);
			
    case council_room:
		i = 0;
      return playCouncilRoom(state, handPos, i, currentPlayer);
    ...
    case mine:
		j = 0;
		i = 0;
      return playMine(choice1, choice2, state, handPos, j, currentPlayer, i);
    ...
    case smithy:
		i = 0;
      return playSmithy(state, handPos, i, currentPlayer);
...
int playSmithy(struct gameState *state, int handPos, int i, int currentPlayer)
{
	//+4 Cards (This is a bug, it should draw only 3 cards)
	for (i = 0; i < 4; i++)
	{
		drawCard(currentPlayer, state);
	}
...
int playAdventurer(struct gameState *state, int drawntreasure, int currentPlayer, int cardDrawn, int z, int temphand[MAX_HAND])
{
	while (drawntreasure<2) {
		...
		if (cardDrawn == silver || cardDrawn == gold) // Removed the cardDrawn == copper as that makes the card not work for that treasure
			drawntreasure++;
		else {
...
int playCouncilRoom(struct gameState *state, int handPos, int i, int currentPlayer)
{
	...
	for (i = 0; i < state->numPlayers; i++)
	{
		if (i == currentPlayer) // Changed != to == making only the currentPlayer get another card
		{
			drawCard(i, state);
		}
	}
